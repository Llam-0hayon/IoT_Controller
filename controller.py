  ...
  #                      "action":{"message":"its too hot, turn on ac", "topic":"room/AC", "value":"on"}
   #             }
        ...
        for rule in IoT_controller.rules:
             if topic == rule ["condition"]["action"]and condition_met(value,rule["condition"]["comparison"],rule["condition"]["value"]):
            	#action
            	print(rule["action"]["message"])
            	IoT_Controller.client.publish(rule["action"]["topic"],rule["action"]["value"])
